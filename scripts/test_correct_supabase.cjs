...
function postToSupabaseNative(key, value) {
  return new Promise((resolve) => {
    ...
    const req = https.request({
      ...
      res.on('data', c => b += c);
      res.on('end', () => {
        console.log(`POST [${key}] status:`, res.statusCode, b || 'OK');
...
async function main() {
  console.log('Probando getFromSupabaseNative para attendance_logs...');
  ...
  console.log('Probando postToSupabaseNative para test_key...');
  const ok = await postToSupabaseNative('test_key', [{ test: true }]);

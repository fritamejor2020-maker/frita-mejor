...
async function runFetch5000() {
  const acsUrl = '/ISAPI/AccessControl/AcsEvent?format=json';

  // 1. Obtener totalMatches global o con major: 5, minor: 38
  const reqInit = JSON.stringify({ AcsEventCond: { searchID: "1", searchResultPosition: 0, maxResults: 10, major: 0, minor: 0 } });
  ...
  console.log(`📌 Total de eventos en memoria del biométrico (totalMatches): ${totalMatches}`);

  const startPos = Math.max(0, totalMatches - 5000);
  console.log(`🚀 Escaneando desde posición ${startPos} hasta ${totalMatches} (últimos 5000 eventos)...`);
  ...
  while (pos < totalMatches) {
    const payload = JSON.stringify({ AcsEventCond: { searchID: "1", searchResultPosition: pos, maxResults: pageSize, major: 0, minor: 0 } });
    try {
  ...
  console.log(`✅ Escaneo completado. Total eventos leídos: ${allEvents.length}`);

  // 2. Filtrar descartando completamente attendanceStatus === "undefined" o vacío
  ...
  console.log(`\n🌟 EVENTOS ENCONTRADOS CON ATTENDANCE STATUS VÁLIDO (IGNORANDO UNDEFINED): ${filteredEvents.length}`);
  ...
  const mappedLogs = filteredEvents.map(ev => {
    ...
    return {
      ...
      branchId: 'BRANCH-001',
      terminalId: 'TERM-001',
      verifyMethod: ev.minor === 38 ? 'FINGERPRINT' : 'CARD_PASS',
      doorNo: ev.doorNo || 1
    };
  });

  console.log('\n--- DETALLE DE CADA REGISTRO VÁLIDO OBTENIDO ---');
...
runFetch5000().catch(console.error);

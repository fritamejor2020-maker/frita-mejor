...
async function main() {
  // Download only last 500 events (50 requests) for a quick cross-tab
  const acsUrl = '/ISAPI/AccessControl/AcsEvent?format=json';
  
  // First get totalMatches
  ...
  // Download last 200 events
  let posicion = Math.max(0, total - 200);
  let allEvents = [];
  ...
  console.log(`Descargados: ${allEvents.length} eventos (últimos 200)`);

  // Cross-tab: minor code × has employeeNoString
  const crossTab = {};
  allEvents.forEach(ev => {
    ...
    if (hasEmp) {
      ...
      if (crossTab[key].samples.length < 2) {
  ...
  console.log('\n=== CROSS-TAB: minor code × employeeNoString ===');
  Object.entries(crossTab).sort((a, b) => (b[1].withEmployee + b[1].withoutEmployee) - (a[1].withEmployee + a[1].withoutEmployee)).forEach(([key, val]) => {

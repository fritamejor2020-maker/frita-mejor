...
async function checkRealEmployeeEvents() {
  ...
  let posicion = Math.max(0, totalMatches - 1000);
  const pageSize = 30;
  ...
  do {
    ...
    events.forEach(ev => {
      ...
      if (empNo && empNo !== '0' && empNo !== '18446744073709551613') {
        userEventCounts[empNo] = (userEventCounts[empNo] || 0) + 1;
        if ((userEventCounts[empNo] || 0) <= 3) {
          console.log(`User #${empNo} event:`, { time: ev.time, serialNo: ev.serialNo, status: ev.attendanceStatus });
        }
      }
    });
  ...
  console.log('Real User Event Counts (Last 1000 logs):', userEventCounts);
}

checkRealEmployeeEvents();

...
async function cleanStrict() {
  console.log('=== APLICANDO REGLA STRICTA: ELIMINAR CUALQUIER REGISTRO CON attendanceStatus === "undefined" ===');

  for (const key of ['attendance_logs', 'attendance_logs_BRANCH-001']) {
    const logs = await getSupabaseKey(key);
    const filtered = logs.filter(l => {
      const st = String(l.attendanceStatus || '').trim().toLowerCase();
      if (!st || st === 'undefined' || st === 'null') return false;
      return true;
    });

    console.log(`Key "${key}": pasamos de ${logs.length} a ${filtered.length} marcaciones 100% explícitas.`);

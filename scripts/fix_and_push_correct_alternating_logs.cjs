...
const SUPABASE_REST_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVldmNvdG1uZmZmdG9lbHNjanVhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU2NTk3NjMsImV4cCI6MjA9MTIzNTc2M30.c8q811qfvwOvenGk4mwt1HVTBsD7cPYiTM-2orqz3pM'.replace('20=1235763', '2091235763');

function getFromSupabaseNative(key) {
  return new Promise((resolve) => {
    const req = https.request({
      hostname: SUPABASE_REST_HOST, port: 443,
      path: `/rest/v1/app_state?key=eq.${encodeURIComponent(key)}&select=value`, method: 'GET',
      headers: { 'apikey': SUPABASE_REST_ANON_KEY, 'Authorization': `Bearer ${SUPABASE_REST_ANON_KEY}`, 'Accept': 'application/json' }
    }, (res) => {
      let b = ''; res.on('data', c => b += c);
      res.on('end', () => {
        try { resolve(res.statusCode === 200 ? JSON.parse(b)[0]?.value : null); } catch (e) { resolve(null); }
      });
    });
...
async function main() {
  console.log('🚀 Leyendo posiciones 0 a 7800...');
  ...
  // Group by employee and date string to assign alternating ENTRY (checkIn) and EXIT (checkOut)
  const byEmpDay = {};
  ...
  Object.values(byEmpDay).forEach(dayEvents => {
    ...
    dayEvents.forEach((ev, idx) => {
      ...
      // Determine if checkOut: explicit checkOut string OR minor 22 OR odd index (1st punch = checkIn, 2nd punch = checkOut, 3rd = checkIn, 4th = checkOut)
      let isExit = false;
      if (rawStatus === 'checkout' || rawStatus === 'exit' || rawStatus === 'check_out' || rawStatus === 'out' || ev.minor === 22) {
        ...
        // If undefined/generic: 0th punch = ENTRY (checkIn), 1st punch = EXIT (checkOut)
        isExit = idx % 2 === 1;
      }
  ...
  console.log(`📋 Total logs mapeados con alternancia inteligente: ${mappedLogs.length}`);

  // Check Valentina (#12) logs for August 2026
  console.log('\n--- MARCACIONES DE VALENTINA (#12) EN AGOSTO 2026 ---');
  ...
  if (ok1 && ok2) {
    console.log(`\n🎉 ¡ÉXITO TOTAL! ${mappedLogs.length} marcaciones guardadas con alternancia checkIn/checkOut correcta.`);
  }
}

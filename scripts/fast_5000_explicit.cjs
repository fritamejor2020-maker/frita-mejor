...
async function runFast5000() {
  ...
  console.log(`📌 Total matches en memoria: ${totalMatches}`);

  const startPos = Math.max(0, totalMatches - 5000);
  console.log(`🚀 Escaneando de pos ${startPos} a ${totalMatches}...`);
  ...
  console.log(`✅ Eventos leídos en rango pos 21234..26234: ${allEvents.length}`);
  ...
  console.log('\n--- DETALLE DE REGISTROS VÁLIDOS ENCONTRADOS ---');
  mappedLogs.forEach((l, idx) => {
    console.log(`${idx + 1}. Empleado #${l.employeeNo.padEnd(3)} | Serial: ${String(l.serialNo).padEnd(6)} | Fecha/Hora: ${l.timestamp} | Estado: ${l.attendanceStatus} (Raw: "${l.rawAttendanceStatus}")`);
  });
...
runFast5000().catch(console.error);

...
async function testMaxResults() {
  for (const size of [30, 50, 100, 200]) {
    ...
    try {
      const res = await isapiDigestFetchSingle('/ISAPI/AccessControl/AcsEvent?format=json', { method: 'POST', body: payload });
      const data = JSON.parse(res.text || '{}');
      const count = (data.AcsEvent?.InfoList || []).length;
      console.log(`maxResults: ${size} -> returned ${count} items (ok: ${res.ok})`);
    } catch (e) {
      console.log(`maxResults: ${size} -> failed: ${e.message}`);
    }
  }
}

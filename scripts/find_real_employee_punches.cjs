...
async function findRealPunches() {
  ...
  console.log(`Total Matches in Device: ${totalMatches}`);

  // Loop backwards from latest to find real employee punches
  let posicion = Math.max(0, totalMatches - 300);
  let foundEvents = [];

  for (let p = posicion; p < totalMatches; p += 30) {
    const batchCond = { searchID: "1", searchResultPosition: p, maxResults: 30, major: 0, minor: 0 };
    try {
  ...
  console.log(`Found ${foundEvents.length} real employee punches!`);
  console.log(foundEvents.slice(-20));
}

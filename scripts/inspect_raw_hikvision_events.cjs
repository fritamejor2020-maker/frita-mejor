...
async function inspectEvents() {
  ...
  // Fetch last 30 events raw
  const startPos = Math.max(0, totalMatches - 30);
  const batchCond = { searchID: "1", searchResultPosition: startPos, maxResults: 30, major: 0, minor: 0 };
  ...
  console.log('Sample raw events from device:');

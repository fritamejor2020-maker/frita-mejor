...
async function inspectUsers() {
  const pathStr = '/ISAPI/AccessControl/UserInfo/Search?format=json';
  ...
  const res = await isapiDigestFetchSingle(pathStr, { method: 'POST', body: payload });
  if (!res.ok || !res.text) {
  ...
  users.forEach((u, i) => {
    console.log(`User #${i+1}: employeeNo="${u.employeeNo}" | name="${u.name}" | userType=${u.userType}`);
  });
}

...
async function countValidPunches() {
  const pathStr = '/ISAPI/AccessControl/AcsEvent?format=json';

  console.log('Scanning checkIn/checkOut logs for non-empty employee IDs...');

  let totalScanned = 0;
  const userCounts = {};

  // Scan last 1,500 checkIns and checkOuts
  for (const minor of [21, 22]) {
    const initCond = { searchID: "1", searchResultPosition: 0, maxResults: 10, major: 5, minor };
    ...
    for (let p = startPos; p < totalMatches; p += 30) {
      const batchCond = { searchID: "1", searchResultPosition: p, maxResults: 30, major: 5, minor };
      try {
        const res = await isapiDigestFetchSingle(pathStr, { method: 'POST', body: JSON.stringify({ AcsEventCond: batchCond }) });
        if (!res.ok) continue;

        const events = JSON.parse(res.text || '{}').AcsEvent?.InfoList || [];
  ...
  console.log(`Scanned ${totalScanned} checkIn/checkOut logs.`);
  console.log('Punches grouped by Employee ID:', userCounts);
}

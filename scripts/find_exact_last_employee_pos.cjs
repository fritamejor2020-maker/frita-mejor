...
async function main() {
  ...
  // Binary search or linear search between 7000 and 8000 to find the last position with employeeNoString
  let lastValidPos = 0;
  let lastValidEvent = null;

  for (let pos = 7400; pos < 7800; pos += 10) {
    const payload = JSON.stringify({ AcsEventCond: { searchID: "1", searchResultPosition: pos, maxResults: 10, major: 0, minor: 0 } });
    try {
      ...
      batch.forEach((ev, idx) => {
        ...
        if (emp && emp !== '' && emp !== '0' && emp !== '18446744073709551613') {
          lastValidPos = pos + idx;
          lastValidEvent = ev;
        }
      });
    } catch (e) {}
  ...
  console.log(`🎯 Última posición en la memoria con empleado válido: ${lastValidPos}`);
  if (lastValidEvent) {
    console.log(`   Empleado: #${lastValidEvent.employeeNoString} | Fecha: ${lastValidEvent.time} | minor: ${lastValidEvent.minor} | serial: ${lastValidEvent.serialNo}`);
  }
}

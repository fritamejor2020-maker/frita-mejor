...
async function inspectRawRecent() {
  ...
  // Fetch last 30 minor 38 events
  const reqLast = JSON.stringify({ AcsEventCond: { searchID: "1", searchResultPosition: Math.max(0, total38 - 30), maxResults: 30, major: 5, minor: 38 } });
  const resLast = await dig(acsUrl, { method: 'POST', body: reqLast });
  const list = JSON.parse(resLast.text).AcsEvent?.InfoList || [];

  console.log(`\n📋 TODAS LAS PROPIEDADES DE LOS ÚLTIMOS 30 EVENTOS DE EMPLEADOS (MINOR 38):`);
  list.forEach((ev, idx) => {
    console.log(`\n--- Registro #${idx + 1} (Serial: ${ev.serialNo}) ---`);
    ...
    console.log(`  RAW JSON:         ${JSON.stringify(ev)}`);
  });
}

inspectRawRecent().catch(console.error);
